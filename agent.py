# Team name: my machine is not learning
# Team ID: 10
# Team members: 0613246 李泓賢, 0816039 李品慈, 0816203 陳永諭
...
def InitPos(mapStat):
    ...
    current_player = 1
    sheepState = np.zeros((GameMeta.BOARD_SIZE, GameMeta.BOARD_SIZE))
    for i in range(GameMeta.BOARD_SIZE):
        for j in range(GameMeta.BOARD_SIZE):
            if mapStat[i][j] > 0:
                sheepState[i][j] = GameMeta.MAX_SHEEP
                current_player += 1

    state = State(mapState=mapStat, sheepState=sheepState, current_player=current_player)
    ...
    agent.move_to(best_move, msg="best move")
...
def GetStep(playerID, mapStat, sheepStat):
    ...
    for _ in range(GameMeta.PLAYERS-1):
        ...
        if currentPlyer not in sources and currentPlyer not in destinations:
            agent.move_to(Move.freeze())
        # init pos
        elif currentPlyer not in sources:
            agent.move_to(Move(cell=destinations[currentPlyer], init=True), msg="update")
        else:
